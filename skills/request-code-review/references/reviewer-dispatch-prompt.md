...
Fill every `{{placeholder}}` before sending. Never dispatch with a placeholder still in the text — if a value is unknown, ask the user rather than guessing.
...
## Your task
You are a fresh-context reviewer with no memory of how this code was written. Run
`git diff {{base_commit}}..{{head_commit}}` yourself and review the actual diff —
do not trust any summary of it. Scan in this strict priority order and stop
escalating once you've covered every tier; do not skip ahead.
...
## Rules
- Read-only. Do not edit any files — you are reviewing, not fixing.
