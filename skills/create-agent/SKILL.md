...
**Core mindset:** an agent is the _expensive_ primitive. A fresh agent re-derives context cold, burns its own token budget, and adds a coordination boundary you now have to design across. Reach for one only when isolation, parallelism, or a reusable narrow role earns its keep. If the model just needs _instructions_ it can follow inline, that is a **skill**. If a fixed action must happen at a lifecycle point, that is a **hook**. If you need it once, just do it.
...
| The model needs _instructions/knowledge_ it applies inline                              | **Skill** — not an agent               |
...
The _when_ and _who-invokes_ determine the primitive. Read the deep comparison — cost, lifetime, observability, and migration paths — in **[references/primitives.md](references/primitives.md)** before committing.
...
The system prompt **is** the agent — for a subagent it _replaces_ the default Claude Code prompt entirely. This is the craft. Read **[references/system-prompt-craft.md](references/system-prompt-craft.md)** for the full treatment; the skeleton:
...
3. **Boundaries** — what it must _not_ do (scope limits, "don't touch X", "never commit"). Agents over-reach without explicit fences.
...
| Orchestrate sub-work    | add `Agent(worker, researcher)` — allowlist _which_ agents it may spawn |
...
Match the model to the _hardest_ thing the agent must do — not the ceiling, not the floor.
...
- **MCP servers.** `mcpServers` attaches tools inline (connect on start, disconnect on finish) or by reference — keeps MCP tool defs out of the _main_ conversation's context.
...
**Then exercise behavior.** Run the agent on 2–3 representative inputs, including one edge case and one it should _refuse_ or scope out of. Check:
