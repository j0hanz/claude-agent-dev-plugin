...
Ask exactly 3 `AskUserQuestion` prompts, in this exact order, once per init. For each, use the Decision Protocol:

1. **Commit & attribution policy**
   - ✅ **Recommended** — [Strict/Relaxed] based on [common repo patterns].
   - **Alternative** — [Alternative Option] + reason.
   - **Other** — Custom answer.

2. **Project maturity state**
   - ✅ **Recommended** — [Production/Development] based on [signals like version, size].
   - **Alternative** — [Alternative Option].
   - **Other** — Custom answer.

3. **Testing rigor**
   - ✅ **Recommended** — [Always/Touched-files] based on [existing test infra].
   - **Alternative** — [Alternative Option].
   - **Other** — Custom answer.
