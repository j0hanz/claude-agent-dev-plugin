...
def ensure_gh_available(repo_root: Path) -> bool:
    if which("gh") is None:
        ...
        return False
    result = run_command(["gh", "auth", "status"], cwd=repo_root)
    if result.returncode == 0:
        return True
...
def resolve_pr(pr_value: str | None, repo_root: Path) -> str | None:
    if pr_value:
        return pr_value
    result = run_command(["gh", "pr", "view", "--json", "number"], cwd=repo_root)
    if result.returncode != 0:
        message = (result.stderr or result.stdout or "").strip()
        print(message or "Error: unable to resolve current PR.", file=sys.stderr)
        return None
    try:
...
def fetch_checks(pr_value: str, repo_root: Path) -> list[dict[str, Any]] | None:
    ...
    result = run_command(
        ["gh", "pr", "checks", pr_value, "--json", ",".join(fields)], cwd=repo_root
    )
    if result.returncode != 0:
        message = (result.stderr or result.stdout or "").strip()
        available_fields = parse_available_fields(message)
        if available_fields:
            fallback = [f for f in fields if f in available_fields]
            if not fallback:
                print(
                ...
                return None
            result = run_command(
                ["gh", "pr", "checks", pr_value, "--json", ",".join(fallback)],
                cwd=repo_root,
            )
            if result.returncode != 0:
                message = (result.stderr or result.stdout or "").strip()
...
def analyze_check(
    ...
    url = normalize_field(check.get("detailsUrl") or check.get("link") or "")
...
def fetch_run_metadata(run_id: str, repo_root: Path) -> dict[str, Any] | None:
    ...
    result = run_command(
        ["gh", "run", "view", run_id, "--json", ",".join(fields)], cwd=repo_root
    )
    if result.returncode != 0:
        return None
    try:
...
def fetch_run_log(run_id: str, repo_root: Path) -> tuple[str, str]:
    result = run_command(["gh", "run", "view", run_id, "--log"], cwd=repo_root)
    if result.returncode != 0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect failing GitHub PR checks, fetch Actions logs, and extract a failure snippet.",
    ...
    parser.add_argument(
        ...
        help="Maximum lines to include in the log tail or snippet.",
    )
    parser.add_argument(
        "--context",
        type=int,
        default=DEFAULT_CONTEXT_LINES,
        help="Context lines around the failure marker.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if not failing:
        ...
        return 0
    results = [
        analyze_check(check, repo_root, max(1, args.max_lines), max(1, args.context))
