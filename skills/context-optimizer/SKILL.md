...
Phase 1: Diagnose (Run scripts/diagnose_context_bloat.py and /context)
  ...
  -> Phase 3: Action (Run prune_context.py or specify Read offset/limit slices)
...
1. Check for workspace dirtiness and large files:
   `python ${CLAUDE_SKILL_DIR}/scripts/diagnose_context_bloat.py`
...
- **Strategy A: KV Compaction (JSON/Configs)**
  `python ${CLAUDE_SKILL_DIR}/scripts/prune_context.py --to-kv < data.json`
- **Strategy B: Log Truncation (Test Failures/Traces)**
  `python ${CLAUDE_SKILL_DIR}/scripts/prune_context.py --logs < test_output.log`
- **Strategy C: Line Slicing (Large Files)**
  Find target lines with `Grep`, then read ONLY the required lines using the `Read` tool's `offset` and `limit` params.
...
1. Write a flat Key-Value status, including which skill or gate was active so resume doesn't re-derive routing from scratch:
   `python ${CLAUDE_SKILL_DIR}/scripts/prune_context.py --summary --timestamp "$(date -Iseconds)" --current-skill "<skill-name or gate>" --done "completed items" --blocking "blockers" --next-step "next actions" --decisions "key decisions"`
...
- **NEVER** read files larger than 300 lines without specifying `offset`/`limit`. Run `Grep` first.
