...
- **Re-review Cap:** Once the corrective skill (`diagnose`/`refactor`) verifies its fix, control returns to `receive-code-review`, which re-invokes `request-code-review` for a fresh-context re-review of the same range. This cycle is capped at 2 re-reviews before escalating to the user — `receive-code-review`'s own doc states this cap directly so the limit isn't only discoverable via the orchestrator.
